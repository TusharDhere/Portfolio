/* ═══════════════════════════════════════════════════════════
   EMAILJS CONFIGURATION — REQUIRED for direct email sending
   ───────────────────────────────────────────────────────────
   ⚠️ While the 3 values below still say 'YOUR_...', clicking
   "Send Message" opens your mail app as a fallback instead of
   sending silently. Fill these in and the redirect disappears
   completely — the form sends in the background instead.

   See EMAILJS_SETUP.md in this folder for full click-by-click
   steps. Quick version (~3 minutes):
   1. https://www.emailjs.com/ → Sign up free
   2. Email Services → Add New Service → Gmail → connect
      work.tushardhere@gmail.com → copy the Service ID
   3. Email Templates → Create New Template → use these vars
      in the body: {{from_name}} {{from_email}} {{subject}}
      {{message}} → Save → copy the Template ID
   4. Account → General → copy your Public Key
   5. Paste all 3 below
   6. Delete the yellow "Setup required" box in index.html
...
async function sendEmail(e) {
  ...
  const params = {
    // Matches your EmailJS template variables exactly:
    // {{from_email}} -> From Email field, {{name}} & {{time}} & {{message}} -> Content body
    from_name:  document.getElementById('from_name').value.trim(), // kept for mailto fallback below
